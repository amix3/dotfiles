...
local user_opts = {
    ...
    scalefullscreen = 1.25,      -- scaling of the controller when fullscreen
...
local language = {
    ["eng"] = {
        welcome = "{\\fs24\\c&HFFFFFF&}Drop files or URLs to play here.",  -- this text appears when mpv starts
        off = "Off",
        unknown = "unknown",
        none = "none",
...
local osc_styles = {
    ...
    seekbar_fg = "{\\blur0\\bord0\\1c&H929537}",
...
function layouts()
    ...
    lo.geometry = {x = refX, y = refY - 45, an = 5, w = osc_geo.w, h = 16}
    lo.style = osc_styles.seekbar_fg

    lo.slider.pad = 0
    lo.slider.handle_size = 0
    lo.slider.bar_height = 6
...
function show_logo()
    ...
    ass:draw_start()
    ass:move_to(-10, -10)
    ass:line_to(11.65, 2.5)
    ass:line_to(-10, 15)
    ass:draw_stop()

    ass:new_event()
    ass:pos(logo_x, logo_y+75)
    ass:an(8)
